...
data_loc = os.path.join(args.data,"BERT.pkl")
cat = data_loc.split("/")[-2]
# print(cat)
# print(args.data)

logdata = defaultdict(lambda: defaultdict(list))
data = read_pickle(data_loc)

Avg = []
layer_generator = range(13) 
for layer in layer_generator:
    data_x = [x["cls"][layer] for x in data]
    norm = LA.norm(data_x)
    # print(norm)
    Avg.append(norm)
avg_norm = np.mean(np.array(Avg))
# f_ = open("cls_norm","a")
print(cat,Avg)
# print(cat,avg_norm,file=f_)
# f_.close()
