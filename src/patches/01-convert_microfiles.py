...
for layer in tqdm(range(12)):
    for amb_mode in ["ambiguous", "unambiguous"]:
        for f in glob(f"data/Ambiguity/{MODEL}/{amb_mode}_representations/CLS/{layer}/*.npy"):
        ...
        for f in glob(f"data/Ambiguity/{MODEL}/{amb_mode}_representations/Token_Aggregation/Haddamard/{layer}/*.npy"):
            fname = f.split("/")[-1].split(".npy")[0]
            with open(f, "rb") as f:
                v = np.load(f)
                data[fname]["haddamard"].append(v)

        for f in glob(f"data/Ambiguity/{MODEL}/{amb_mode}_representations/Token_Aggregation/Mean/{layer}/*.npy"):
            fname = f.split("/")[-1].split(".npy")[0]
            with open(f, "rb") as f:
                v = np.load(f)
                data[fname]["mean"].append(v)

        for f in glob(f"data/Ambiguity/{MODEL}/{amb_mode}_representations/Token_Aggregation/Sum/{layer}/*.npy"):
            fname = f.split("/")[-1].split(".npy")[0]
            with open(f, "rb") as f:
                v = np.load(f)
...
with open(f"data/ambiguity_{MODEL}.pkl", "wb") as f:
    pickle.dump(data, f)